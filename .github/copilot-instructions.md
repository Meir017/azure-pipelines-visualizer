...
- **`@apv/server`** — Hono HTTP server on port 3001. Proxies Azure DevOps REST API calls, maintains a disk cache (keyed by commit SHA) and in-memory TTL caches for repo metadata and ref resolution. Serves files from local repo mappings when configured in `apv.config.json`.
...
- Configuration is in `apv.config.json` (validated by `apv.config.schema.json`). Local repo mappings use case-insensitive `org/project/repoName` keys.
